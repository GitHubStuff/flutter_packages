...
library network_connection;

import 'dart:async';
import 'dart:io';

/// Represents the status of the data connection.
/// Returned by [NetworkConnectionCheck.connectionStatus]
...
/// i.e. NetworkConnectionCheck() always returns the same instance.
class NetworkConnectionCheck {
  ...
  static final List<AddressCheckObject> DEFAULT_ADDRESSES = List.unmodifiable([
    AddressCheckObject(
  ...
  ///
  /// See [AddressCheckObject] for more info.
  List<AddressCheckObject> addresses = DEFAULT_ADDRESSES;

  /// This is a singleton that can be accessed like a regular constructor
  /// i.e. NetworkConnectionCheck() always returns the same instance.
  factory NetworkConnectionCheck() => _instance;
  ...
  static final NetworkConnectionCheck _instance = NetworkConnectionCheck._();

  /// Ping a single address. See [AddressCheckObject] for
  /// info on the accepted argument.
  Future<NetworkResult> isHostReachable(
    AddressCheckObject options,
  ) async {
    Socket? sock;
    try {
      ...
      sock.destroy();
      return NetworkResult(options, true);
    } catch (e) {
      if (sock != null) sock.destroy();
      return NetworkResult(options, false);
    }
  }
  ...
  List<NetworkResult> get lastTryResults => _lastTryResults;
  List<NetworkResult> _lastTryResults = <NetworkResult>[];
  ...
  Future<bool> get hasConnection async {
    List<Future<NetworkResult>> requests = [];
  ...
  /// [NetworkConnectionStatus.disconnected] otherwise.
  Future<NetworkConnectionStatus> get connectionStatus async {
    return await hasConnection ? NetworkConnectionStatus.connected : NetworkConnectionStatus.disconnected;
  }
  ...
  StreamController<NetworkConnectionStatus> _statusController = StreamController.broadcast();

  /// Subscribe to this stream to receive events whenever the
  /// [NetworkConnectionStatus] changes. When a listener is attached
  /// a check is performed immediately and the status ([NetworkConnectionStatus])
  ...
  ///
  /// ```dart
  /// var listener = NetworkConnectionCheck().onStatusChange.listen((status) {
  ///   switch(status) {
  ...
  Stream<NetworkConnectionStatus> get onStatusChange => _statusController.stream;
...
/// If [AddressCheckObject.port]
/// or [AddressCheckObject.timeout] are not specified, they both
/// default to [NetworkConnectionCheck.DEFAULT_PORT]
/// and [NetworkConnectionCheck.DEFAULT_TIMEOUT]
class AddressCheckObject {
  ...
  AddressCheckObject(
    this.address, {
    this.port = NetworkConnectionCheck.DEFAULT_PORT,
    this.timeout = NetworkConnectionCheck.DEFAULT_TIMEOUT,
  });
...
class NetworkResult {
  final AddressCheckObject options;
  ...
  @override
  String toString() => "NetworkResult($options, $isSuccess)";
}
